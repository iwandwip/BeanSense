...
class RBFSVMGridSearchModel:
    def __init__(self, C_range=np.logspace(-2, 2, 5), gamma_range=np.logspace(-4, 0, 5), random_state=42):
    ...
    def train(self, X, y, use_kfold=False, n_splits=5, test_size=0.2, inner_cv=3):
        ...
        if use_kfold:
            ...
            for fold, (train_idx, test_idx) in enumerate(kf.split(X, y), 1):
                ...
                print(f"\n[FOLD {fold}] Best parameters: {self.best_params}")
            ...
            print(f"\nFinal model trained with best parameters: {self.best_params}")
        else:
            X_train, X_test, y_train, y_test = train_test_split(
    ...
    def predict_custom_input(self, input_string):
        ...
        try:
            ...
            prediction_idx = self.model.predict(X_scaled)
    ...
    def load_model(self, filepath):
        ...
        self.C_range = model_data.get('C_range', self.C_range)
        self.gamma_range = model_data.get('gamma_range', self.gamma_range)
        ...
        while True:
            ...
            if mode == "1":
                ...
                if X is not None and y is not None:
                    ...
                    model_file = f"model/rbf_svm_gs_model_{dataset_choice}.pkl"
                ...
                model_questions = [
                    inquirer.List('model',
                                  ...
                                  choices=[
                                      ('4 sensor model (rbf_svm_gs_model_4.pkl)', '4'),
                                      ('6 sensor model (rbf_svm_gs_model_6.pkl)', '6'),
                                      ('8 sensor model (rbf_svm_gs_model_8.pkl)', '8')
                                  ],
                                  )
                ]
